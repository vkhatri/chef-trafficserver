...
storage = node['trafficserver']['config']['storage'].empty? ? ["#{node['trafficserver']['storage_dir']} 256M"] : node['trafficserver']['config']['storage']

template ::File.join(node['trafficserver']['conf_dir'], 'storage.config') do
